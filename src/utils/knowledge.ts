...
class HybridSimilarity {
  ...
  constructor(documents: string[], titles?: string[]) {
    ...
    // 计算平均文档长度
    const totalLen = this.docTokens.reduce(
...
export class Knowledge {
  ...
  private parseBigSearchHttpResponse(
    response: Response,
    rawText: string
  ): BigSearchResponse {
...
export function filterAndSortRankingList(
  ...
  return list
    .filter(
      (item) =>
        item.embed != null &&
        item.embed.dimension > 0 &&
        item.embed.similarity >= threshold
    )
...
function rankRankingListBySimilarity(
  ...
  const withDetail = rankingList.map((item, i) => {
    ...
    const embed: LlmRankingEmbed = {
      dimension: docTexts.length,
      similarity: detail.combined,
    };
    return { item: { ...item, embed }, detail, index: i };
  });

  // 按综合分数排序
  const sorted = [...withDetail].sort(
    (a, b) => b.detail.combined - a.detail.combined
  );
  ...
  debugLog(
    `Knowledge ranking: ${rankingList.length} candidates, ${passedCount} passed (threshold=${threshold}), returning ${ranked.length}`
  );
  return ranked;
}
