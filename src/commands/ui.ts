...
function moveScreenshotToDestination(sourcePath: string, localPath: string): void {
...
async function tryCreateRemoteScreenshot(
  ...
  try {
    ...
    result = {
      ...
      stderr: (error as { stderr?: string; message?: string }).stderr ??
...
uiCommand
  ...
  .option('--device <name|serial>', 'Target device name or serial; required when multiple devices are connected')
  .option('--display <displayId>', 'Target display id')
  .option('--path <path>', 'Existing directory or PNG file path whose parent exists')
