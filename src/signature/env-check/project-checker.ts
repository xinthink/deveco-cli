...
export class ProjectChecker {
  ...
  get project(): Project {
    if (!this._project) {throw new Error('Project not initialized. Call checkProjectDir first.');}
    return this._project!;
  }

  /**
   * 场景 10：检查当前目录是否为有效的 HarmonyOS 项目。
  ...
  checkProjectDir(fail: (msg: string) => CheckResult): CheckResult {
    try {
      ...
      return fail('Not in a valid project directory (project-level build-profile.json5 not found).');
    }
  }
  ...
  checkAtomicService(): CheckResult {
    if (this._project!.isAtomicService()) {
      return {
        passed: false,
        message: 'AtomicService projects are not yet supported. Please configure signing manually.',
      };
    }
    return { passed: true, message: '' };
  }
