...
**deveco-cli** wraps the DevEco Studio toolchain (`ohpm`, `hvigor`, `hdc`, `emulator`, `hilog`, bundled `node` + JBR + SDK) plus Huawei Developer login, the HMOS skills installer, and a project-scaffolding template engine — all behind a single `devecocli` binary, with no need to set `PATH` / `DEVECO_SDK_HOME` / `JAVA_HOME`.
...
│   └── doc.ts                # Local HarmonyOS docs search/read/catalog
...
├── server.ts                 # CodegenieMcpServer — stdio MCP server orchestrator
...
- **`commands/run.ts`** — Auto-selects the runnable module (`entry`/`feature`/`shared`) and the device (name substring or exact serial), then installs HSP deps + the main `.hap` via `HdcAdapter.installApp` (which `hdc file send`s the artifacts to a temp dir and runs `hdc shell bm install -p`), and launches the ability (defaults to `mainElement` from `module.json5`). Supports `--build-mode` (defaults to `debug`) and `--skip-build` to deploy existing artifacts without rebuilding.
...
- **`server.ts`** (`CodegenieMcpServer`) — orchestrator; reads `PROJECT_PATH`, `DEVECO_PATH`, `NODE_MAX_OLD_SPACE_SIZE` (default 8192), `DEBUG` from env.
...
- **MCP global / project-level**: `--mcp` without `--project` configures global MCP for agents with `supportsGlobal` flag (e.g. opencode); other agents emit a consolidated error prompting `--project`. `--mcp --project <path>` configures project-level MCP for all agents. `PROJECT_PATH` in project-level config is the absolute path of the project; in global config it is `.` (OpenCode) or `${workspaceFolder}` (other agents).
