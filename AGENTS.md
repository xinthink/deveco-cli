...
- **MCP global / project-level**: `--mcp` without `--project` configures global MCP for all agents (writes to each agent's global config path). `--mcp --project <path>` configures project-level MCP for most agents (writes to each agent's project config path). Note: Qoder does not support project-level MCP; use `--mcp --agent qoder` for global configuration instead.
