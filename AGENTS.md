...
- **`commands/create.ts`** — Scaffolds a new application project. Requires `--project-path` and `--app-name`; derives `--bundle-name` as `com.example.<appname-lowercase>` when missing; `--api-level` is validated to `17`–`23` or auto-detected via `ToolProvider.detectApiLevel()`. Delegates the file copy + `sdkVersion` / `modelVersion` rendering to `utils/template-provider.ts`, then runs an integrity check on `REQUIRED_FILES`.
...
- **`commands/skills.ts`** + **`skills/`** — `list` / `find` / `add` / `remove`. Downloads skill `.zip`s and extracts them into per-agent paths defined in `config/constants.ts → AGENT_SKILLS_CONFIG` (e.g. `~/.claude/skills/`, `~/.cursor/skills-cursor/`) and / or `<project>/.deveco/skills/`. With neither `--agent` nor `--project`, operates on every detected agent.
